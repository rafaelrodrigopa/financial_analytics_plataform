...
class IncomeStatementSchema(BaseFinancialSchema):
    ...
    revenue: Optional[float] = Field(default=None, description="Receita Total")
    grossProfit: Optional[float] = Field(default=None, description="Lucro Bruto")
    netIncome: Optional[float] = Field(default=None, description="Lucro Líquido")
...
class BalanceSheetSchema(BaseFinancialSchema):
    ...
    totalAssets: Optional[float] = Field(default=None, description="Ativo Total")
    totalLiabilities: Optional[float] = Field(default=None, description="Passivo Total")
    totalStockholdersEquity: Optional[float] = Field(default=None, description="Patrimônio Líquido")
...
class CashFlowSchema(BaseFinancialSchema):
    ...
    operatingCashFlow: Optional[float] = Field(default=None, description="Fluxo de Caixa Operacional")
    freeCashFlow: Optional[float] = Field(default=None, description="Fluxo de Caixa Livre")
...
class CompanyProfileSchema(BaseFinancialSchema):
    ...
    """
    companyName: Optional[str] = Field(default=None, description="Nome da Razão Social")
    industry: Optional[str] = Field(default=None, description="Indústria / Ramo de Atuação")
    sector: Optional[str] = Field(default=None, description="Setor Econômico")
...
class QuoteSchema(BaseFinancialSchema):
    ...
    """
    price: Optional[float] = Field(default=None, description="Preço de Fechamento / Cotação Atual")
...
def validate_dataframe(df: pd.DataFrame, schema_cls: Type[BaseModel]) -> pd.DataFrame:
    ...
    for idx, record in enumerate(records):
        try:
            ...
            logger.warning(f"Linha {idx} ({symbol}) violou o contrato de schema {schema_cls.__name__}: {err.errors()[0].get('msg')}")

    if invalid_count > 0:
        logger.warning(f"Validação de Schema {schema_cls.__name__}: {invalid_count} registros inválidos foram ignorados de {len(records)} totais.")
    else:
        logger.info(f"Validação de Schema {schema_cls.__name__}: 100% dos {len(records)} registros validados com sucesso!")
