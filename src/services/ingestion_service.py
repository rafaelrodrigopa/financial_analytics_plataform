...
class IngestionService:
    ...
    def run_landing_ingestion(self, limit_statements: int = 5) -> None:
        ...
        logger.info(
            f"Iniciando ingestão de {total_symbols} empresas para a camada Landing (Dataset: '{settings.LANDING}')"
        )
    ...
    def _upload_combined(self, df_list: list[pd.DataFrame], table_id: str) -> None:
        ...
        logger.info(
            f"Carregando {len(combined_df)} registros validados (com 'ingested_at') na tabela 'landing.{table_id}'..."
        )

        self.bq.upload_dataframe(
            dataframe=combined_df, dataset_id=settings.LANDING, table_id=table_id, write_disposition="WRITE_TRUNCATE"
        )
